...
bool array_shiftr(array * a, size_t indx, size_t len, size_t off, void ** buff, size_t buff_len, ccds_error * e) {
    ...
    size_t num_copy = a->capacity - (indx + off);
    if(indx + off + len >= a->capacity || buff_len > off) {
        log_error("Capacity: %lu, indx: %lu, off: %lu, buff_len: %lu", a->capacity, indx, off, buff_len);
        ccds_rwlock_wunlock(a->buff_lock);
        return false;
    }
    
    log_trace("capacity: %lu, indx: %lu, off: %lu, buff_len: %lu num_copy: %lu", a->capacity, indx, off, buff_len, num_copy);

    void * tmp[buff_len];
    memset(tmp, 0, buff_len);
    for(size_t i = 0; i < off; i++) {
        if(i < buff_len){
            tmp[i] = a->buffer[indx + num_copy + i];
        }
    }

    /* Copy memory into new place */
    memcpy(&(a->buffer[indx + off]), &(a->buffer[indx]), num_copy * sizeof(void *));    
